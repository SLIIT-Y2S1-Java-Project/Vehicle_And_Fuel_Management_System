...
<html lang="en">
  ...
  <body>
      ...
      <div class="row">
        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 pt-3 px-4" style="margin-left:200px;">
         ...
         <div style="display: flex;">
        	 <div style="width:55%; height: 300px;margin-top: 86px;">
          ...
          <h2 style="display: inline-block">Sells Details</h2>
          ...
          <div class="table-responsive">
            <table class="table table-striped table-sm">
              ...
              <tbody>
              <c:forEach var="cus" items="${fuelsellDetails}">
					<c:set var="id" value="${cus.id}"/>
					<c:set var="type" value="${cus.type}"/>
					<c:set var="liters" value="${cus.liters}"/>
					<c:set var="date" value="${cus.date}"/>
					<c:set var="NoOfCustomers" value="${cus.noOfCustomers}"/>
					<c:set var="branch" value="${cus.branch}"/>
   ...
   <script>window.jQuery || document.write('<js/vendor/jquery-slim.min.js"><\/script>')</script>
    ...
    <script>
		const xValues = ["Colombo", "Gampaha", "Nittambuwa", "Kuruunagala", "Kandy"];
		const yValues = [42244, 42243, 42243, 42240, 42242, 42235];
		const barColors = "#45B8E3";
		...
		new Chart("myChart4", {
			...
			title: {
			display: true,
			text: "Fuel Inventory Of Each Branches"
			}
			}
			});
	</script>
	...
	<script>
			const xArray = ["Colombo", "Gampaha", "Nittambuwa", "Kuruunagala", "Kandy"];
			const yArray = [24256, 24224, 42242, 35334, 24242];
			
			const layout = {title:"Customers Of Each Branches"};
